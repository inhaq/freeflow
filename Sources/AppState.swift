...
import os
import os.log
private let recordingLog = OSLog(subsystem: "com.inhaq.fluent", category: "Recording")
...
enum AppBuild {
    static var isDevBundle: Bool {
        (Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String) == "Fluent Dev"
    }
}
...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    /// URL of the flag file written while Fluent is actively recording.
    ///
    ...
    ///
    /// Path: `~/Library/Application Support/Fluent/is-recording`
    /// (or `Fluent Dev/is-recording` when running the dev bundle).
    static func recordingStateFlagURL() -> URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let appName = Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "Fluent"
    ...
    private static let recordingStateFlagQueue = DispatchQueue(
        label: "com.inhaq.fluent.recording-state-flag"
    )
    ...
    private func beginCriticalDictationActivity() {
        ...
        ProcessInfo.processInfo.disableAutomaticTermination("Fluent dictation in progress")
    ...
    private func endCriticalDictationActivity() {
        ...
        guard automaticTerminationDisabled else { return }
        ProcessInfo.processInfo.enableAutomaticTermination("Fluent dictation in progress")
